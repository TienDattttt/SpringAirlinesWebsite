...
public class AdminRouteModifyService {
	...
	public void updateFlight(Long flightId, Date newDepartureDate, Time newDepartureTime, Long newPlaneID)
		...
		planeService.updateAvailableQuantity(oldPlaneID, false);
		planeService.updateAvailableQuantity(newPlaneID, true);

		// update flights
		LocalDateTime newArrivalDateTime = DateTimeUtils.calculateDateTimeAfterDuration(newDepartureDate,
				newDepartureTime, flight.getDuration());
		adminRouteRepository.updateFlight(flight, newDepartureDate, newDepartureTime, newArrivalDateTime, status,
				newPlaneID);
	...
	public void validateFlightsUpdate(List<Long> flightIds, List<Date> departureDates, List<Time> departureTimes,
			List<Long> planeIDs) {
		if (flightIds.size() > 1) {

			if (flightIds.size() != departureDates.size() || flightIds.size() != departureTimes.size()) {
				throw new IllegalArgumentException(
						"Number of flight IDs, departure dates, and departure times must be equal.");
			}

			LocalDateTime flight1ArrivalDateTime = DateTimeUtils.calculateDateTimeAfterDuration(
					departureDates.get(0), departureTimes.get(0),
					flightService.getFlightByID(flightIds.get(0)).getDuration());
			...
			if (!flight2DepartureDateTime.isAfter(flight1ArrivalDateTime)) {
				throw new IllegalArgumentException(
						"The departure time of Flight 2 must be later than the arrival time of Flight 1.");
			}

			if (flight2DepartureDateTime.isAfter(flight1ArrivalDateTime.plus(Duration.ofHours(8)))) {
				throw new IllegalArgumentException(
						"The departure time of Flight 2 must occur within 8 hours following the arrival time of Flight 1.");
			}
			...
			Plane flight1OldPlane = planeService
					.getPlaneByID(flightService.getFlightByID(flightIds.get(0)).getPlane().getId());
			Plane flight2OldPlane = planeService
					.getPlaneByID(flightService.getFlightByID(flightIds.get(1)).getPlane().getId());

			if ((flight1NewPlane.getId() != flight1OldPlane.getId())
					|| (flight2NewPlane.getId() != flight2OldPlane.getId())) {
				...
				if (flight1NewPlane.getNumOfRows() * flight1NewPlane.getNumOfSeatsPerRow() != flight2NewPlane
						.getNumOfRows() * flight2NewPlane.getNumOfSeatsPerRow())
			...
			if ((flight1NewPlane.getId() != flight1OldPlane.getId())) {
				...
				if (flight1NewPlane.getAvailableQuantity() < 1)
					throw new IllegalArgumentException("Vui lòng chọn máy bay có sẵn cho chuyến bay.");
			}
		}
