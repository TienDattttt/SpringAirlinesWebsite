				...
				<div class="card-body p-4">
					...
					<div class="row d-flex justify-content-center">
						<div class="col-lg-10">
							...
							<form id="form" th:action="@{/registration/ProcessUserRegistration}" th:object="${user}"
								method="post">

								<div class="form-floating mb-3">
									<input name="userName" type="text" th:field="*{userName}" class="form-control"
										placeholder="Username" maxlength="20" />
									<label for="userName">Tên đăng nhập*</label>
									<p th:if="${#fields.hasErrors('userName')}" th:errors="*{userName}"
										class="text-danger error-message">Sai Tên đăng nhập</p>
								</div>
								...
								<p th:if="${#fields.hasErrors('password')}" th:errors="*{password}"
									class="text-danger error-message">Sai mật khẩu</p>

								<div class="form-floating mb-3">
									<input name="email" type="text" th:field="*{email}" class="form-control"
										placeholder="Email" maxlength="40" /> <label for="email">Email*</label>
									<p th:if="${#fields.hasErrors('email')}" th:errors="*{email}"
										class="text-danger error-message">Email không phù hợp</p>
								</div>

								<div class="text-center">
									<button type="submit" onclick="load(event)" data-mdb-button-init
										data-mdb-ripple-init class="btn btn-outline-primary col-12 rounded-0">
										<span id='spinner' style="display: none"
											class="spinner-border spinner-border-sm" role="status"
											aria-hidden="true"></span> Đăng Ký
									</button>
								</div>

								<div th:if="${#fields.hasErrors('global')}" class="text-center mt-2">
