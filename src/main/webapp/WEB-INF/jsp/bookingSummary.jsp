	...
	<div layout:fragment="content">
		...
		<div class="container padding-bottom-3x mb-1">
			<div class="card mb-1 border-0">
				<h2 class="fw-bold mb-1 text-center">Tóm Tắt Đặt Chỗ</h2>
				<div class="card-body">
					<div
		...
		<div id="countdown-alert" class="alert alert-primary w-40" role="alert">
			<input type="hidden" id="seatsReserveTime" th:value="${SeatsReservationTime}" /> Ghế của bạn được giữ trong
			<span id="countdown"></span> phút!
		</div>
				...
				<ul class="list-inline items">
					...
					<li th:each="flight : ${route.flights}" th:if="${flightStat.index == 0}"
						...
						<div class="px-3 text-muted">
							...
							<br /> <small th:text="${'Mã Chuyến Bay: ' + flight.id}"></small>
							<div class="pt-2">
					...
					<div th:each="passenger, passengerIndex : *{passengerList}" class="col-lg-4 col-md-4 mb-3">
						<div class="card card-pass" style="max-width: 450px;">
							<div class="card-body">
								<div class="row">
									<div class="col">
										<div class="form-floating mb-3">
											...
											<label th:for="${'passengerName' + passenger.passportID}">Tên Hành
												Khách</label>
										</div>
									</div>
									<div class="col">
										<div class="form-floating mb-3">
											...
											<input disabled type="text" class="form-control"
												...
												th:value="${passenger.passportID}">
											<label th:for="${'passenger' + passenger.passportID}">Số Hộ Chiếu</label>
										</div>
									</div>
								</div>
								<div th:each="flightId : ${passenger.flightAndSeats.keySet()}">
									...
									<div class="mb-3">
										<h5 class="text-primary strong">
											Mã Chuyến Bay: <span th:text="${flightId}"></span>
										</h5>
										<p>
											<strong>Ghế:</strong> <span
										...
										<table class="table">
											<tbody class="text-muted">
												<tr th:each="luggageId : ${passenger.flightAndLuggage.get(flightId)}">
													...
													<td th:each="luggage : ${LuggageList}"
														th:if="${#strings.equals(#strings.toString(luggage.id), #strings.toString(luggageId))}">
														<span
															th:text="${'$' + #numbers.formatDecimal(luggage.price, 0, 'COMMA', 0, 'POINT')}"></span>
													</td>
												</tr>
											</tbody>
										</table>
								...
								<div class="row">
									<div class="col">
										<strong>Tổng giá tiền hành lý:</strong> <span class="text-info float-end"
								...
								<div class="row">
									<div class="col">
										...
										<strong>Giá vé:</strong> <span class="text-info float-end"
											th:if="${totalTicketPriceForPassenger != null}"
											th:text="${'$' + #numbers.formatDecimal(totalTicketPriceForPassenger, 0, 'COMMA', 0, 'POINT')}">
										</span>
		...
		<div class="text-center">
			<section>
				<strong>Tổng giá vé đặt chỗ:</strong> <span class="text-info" th:if="${totalPrice != null}"
					th:text="${'$' + totalPrice}">
				</span>
			</section>
			...
			<form th:action="@{/bookingSummary/ProcessBooking}" method="post">
				<div class="mb-2 mt-4">
					...
					<button id="submitBtn" th:if="${success_message != null}" disabled type="submit"
						onclick="load(event)" data-mdb-button-init data-mdb-ripple-init
						...
						<span id='spinner' style="display: none" class="spinner-border spinner-border-sm" role="status"
							aria-hidden="true"></span> Đặt chuyến bay!
					</button>
					<button id="submitBtn" th:if="${success_message == null}" type="submit" onclick="load(event)"
						data-mdb-button-init data-mdb-ripple-init class="btn btn-primary rounded-0 p-2 col-2">
						<span id='spinner' style="display: none" class="spinner-border spinner-border-sm" role="status"
							aria-hidden="true"></span> Đặt chuyến bay!
					</button>
				</div>
